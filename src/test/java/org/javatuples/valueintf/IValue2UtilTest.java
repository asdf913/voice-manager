...
class IValue2UtilTest {

	@Test
	void testGetValue2() throws Throwable {
		//
		...
		for (final ClassInfo classInfo : classInfos) {
			...
			try {
				//
				if (isAssignableFrom(IValue2.class, Class.forName(name = HasNameUtil.getName(classInfo)))
						&& !(clz = Class.forName(name)).isInterface() && !Modifier.isAbstract(clz.getModifiers())) {
					//
					final IValue2<?> iValue2 = cast(IValue2.class, Narcissus.allocateInstance(clz));
					//
					System.out.println(name);
					//
					Assertions.assertDoesNotThrow(() -> IValue2Util.getValue2(iValue2), name);
					//
				} // if
					//
