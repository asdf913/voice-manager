...
public class OtoYakuNoHeyaYomikataJitenYuryodoYomikataJitenMultimapFactoryBean
	...
	private static Multimap<String, String> createMultimap2(final String s1, final String s2,
		...
		final int size = MultimapUtil.size(mm1);
		//
		if ((mm2 = toMultimap11(s1, s2)) != null) {
			//
			MultimapUtil.putAll(mm1, mm2);
			//
		} // if
			//
		if (MultimapUtil.size(mm1) == size && (mm2 = toMultimap12(s1, s2)) != null) {
			//
			MultimapUtil.putAll(mm1, mm2);
			//
		} // if
	...
	private static Multimap<String, String> toMultimap1(final String s1, final String s2) {
		...
		Matcher m1;
		//
		if (Objects.equals(List.of(UnicodeBlock.CJK_UNIFIED_IDEOGRAPHS), ub)
				&& Objects.equals(List.of(UnicodeBlock.BASIC_LATIN), getUnicodeBlocks(s2))) {
			//
			return multimap;
			//
		} // if
			// if
		if (Objects.equals(List.of(UnicodeBlock.CJK_UNIFIED_IDEOGRAPHS), ub)
				&& Objects.equals(List.of(UnicodeBlock.HIRAGANA), getUnicodeBlocks(s2))) {
			//
	...
	private static Multimap<String, String> toMultimap2(final String s1, final String s2) {
		...
		Multimap<String, String> multimap = null;
		//
		if (Boolean.logicalAnd(Objects.equals(Arrays.asList(UnicodeBlock.CJK_UNIFIED_IDEOGRAPHS, UnicodeBlock.HIRAGANA),
				getUnicodeBlocks(s1)), Objects.equals(List.of(UnicodeBlock.HIRAGANA), getUnicodeBlocks(s2)))) {
			//
	...
	private static Multimap<String, String> toMultimap3(final String s1, final String s2) {
		...
		if (Util.matches(m1 = Util
			...
			for (int j = 1; j <= orElse(min(mapToInt(Stream.of(m1, m2), Util::groupCount)), 0); j++) {
				...
				&& Util.groupCount(m1) > 1 && Objects.equals(List.of(UnicodeBlock.HIRAGANA), getUnicodeBlocks(s2))) {
			//
			MultimapUtil.put(multimap = ObjectUtils.getIfNull(multimap, LinkedHashMultimap::create),
	...
	private static Multimap<String, String> toMultimap4(final String s1, final String s2) {
		...
		if (Util.matches(m1 = Util.matcher(
			...
			for (int j = 1; j <= orElse(min(mapToInt(Stream.of(m1, m2), Util::groupCount)), 0); j++) {
				...
				&& Util.groupCount(m1) > 0 && Objects.equals(List.of(UnicodeBlock.HIRAGANA), getUnicodeBlocks(s2))) {
			//
			MultimapUtil.put(multimap = ObjectUtils.getIfNull(multimap, LinkedHashMultimap::create), Util.group(m1, 1),
					s2);
			//
	...
	private static Multimap<String, String> toMultimap5(final String s1, final String s2) {
		...
		if (Util.matches(m1 = Util.matcher(Pattern.compile(
			...
			for (int j = 1; j <= orElse(min(mapToInt(Stream.of(m1, m2), Util::groupCount)), 0); j++) {
				...
				//
		} else if (Util.matches(Util.matcher(
				Pattern.compile("^(\\p{InCJKUnifiedIdeographs}+)(\\p{InKatakana}+)(\\p{InCJKUnifiedIdeographs}+)$"),
				s1)) && Objects.equals(List.of(UnicodeBlock.HIRAGANA), getUnicodeBlocks(s2))) {
			//
		...
		if (Util.matches(m1 = Util
			...
			for (int i = 1; i <= orElse(min(mapToInt(Stream.of(m1, m2), Util::groupCount)), 0); i++) {
				...
				if (Boolean.logicalOr(
						!Objects.equals(List.of(UnicodeBlock.CJK_UNIFIED_IDEOGRAPHS),
	...
	private static String substringBetween(@Nullable final String str, final String open, final String close) {
		//
		return Boolean.logicalAnd(StringsUtil.startsWith(Strings.CS, str, open),
				StringsUtil.endsWith(Strings.CS, str, close)) ? StringUtils.substringBetween(str, open, close) : str;
		//
	}
